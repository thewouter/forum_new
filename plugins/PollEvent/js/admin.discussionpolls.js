/* 	Copyright 2013-2017 Wouter van Harten
...
jQuery(document).ready(function($) {
  ...
  // If there is input in the first question textbox onload, this is an existing poll
    console.log($('#DP_Questions01').val())
  if ($('#DP_Questions01').val()) {
    ...
    // Hide the form initially
	  $('#DP_Questions0').val('Kom je ook?')
    $('#DP_Form').hide();

    // Check attach poll initially
    $('#Form_DP_Attach').prop('checked', false);
    //$('#DP_Form').slideDown();
  }

  if (!$('#Form_Name').val()){
    $('#Form_Name').val('Opkomst PERSOON');
    $('#Form_Body').val('Opkomst van PERSOON, Geef even aan of je komt. \n\nTot dan!');
  }

  $('#Form_DiscussionEventDates').val($('#Form_DiscussionEventDatesHidden').val());
