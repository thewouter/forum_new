...
function DiscussionPollAnswerForm($PollForm, $Poll, $PartialAnswers) {
  ...
  echo $PollForm->Open(array('action' => Url('/poll/submit/'), 'method' => 'post', 'ajax' => TRUE));
  ...
  foreach($Poll->Questions as $Question) {
    ...
    foreach($Question->Options as $Option) {
      if(GetValue($Question->QuestionID, $PartialAnswers) == $Option->OptionID) {
        ...
        if ($Question->MultipleVote){
      		echo Wrap($PollForm->checkBox('DP_Answer' . $Option->OptionID, $Option->Title, array('Value' => $Option->OptionID, 'checked' => 'checked')), 'li');
        } else {
      	...
      	if ($Question->MultipleVote){
      		echo Wrap($PollForm->checkBox('DP_Answer' . $Option->OptionID, $Option->Title, array('Value' => $Option->OptionID)), 'li');
      	} else {
...
DiscussionPollAnswerForm($this->data('PollForm'), $this->data('Poll'), $this->data('PartialAnswers'));
