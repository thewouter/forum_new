...
function DPRenderQuestionField($PollForm, $Index = 0, $Question = NULL, $Disabled = array()) {
  echo '<fieldset id="DP_Question' . $Index . '" class="DP_Question">';
    ...
    // Render 3 standard options in the question doesn't have any options
    if(is_null($Question)) {
      DPRenderOptionField($PollForm, 0);
      DPRenderOptionField($PollForm, 1);
      DPRenderOptionField($PollForm, 2);
    }
    else {
      $j = 0;
      echo $PollForm->Hidden("QuestionID",array('name' => "QuestionID".$Index,'value' =>$Question->QuestionID));
...
function DPRenderOptionField($PollForm, $OIndex = 0, $QIndex = 0, $Option = NULL, $Disabled = array()) {
  $answers = array('Ja, natuurlijk kom ik ook.', 'Ik ben wat later.', 'Nee, helaas kan ik er deze keer niet bij zijn.');
  ...
  echo Wrap(
          $PollForm->TextBox(
            ...
            array_merge(
              $Disabled,
              array(
                'value' => (is_null($Option)) ? $answers[$OIndex] : $Option->Title,
